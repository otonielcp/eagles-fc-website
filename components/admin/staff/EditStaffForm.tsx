...
import { updateStaff, uploadStaffImage } from "@/actions/staff";
import { Staff, StaffFormData } from "@/types/team";
...
export default function EditStaffForm({ staff, teamId }: EditStaffFormProps) {
  ...
  const [formData, setFormData] = useState<StaffFormData>({
    ...
    role: staff.role,
    dateOfBirth: format(new Date(staff.dateOfBirth), "yyyy-MM-dd"),
    ...
    order: staff.order,
    socialMedia: {
      instagram: staff.socialMedia.instagram,
      twitter: staff.socialMedia.twitter,
      facebook: staff.socialMedia.facebook
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateStaff(staff._id, formData);

      if (result.success) {
        toast.success(result.message);
        router.push(`/admin/teams/${teamId}/staff`);
      } else {
